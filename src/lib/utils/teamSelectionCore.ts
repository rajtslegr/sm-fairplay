...
const calculateMatchHistoryStats = (matchHistory: Match[]): MatchHistoryStats => {
...
const selectTeamsWithoutHistoryWithDebug = (
	...
	teams: [Player[], Player[]];
	debugInfo: Omit<SelectionStats, 'sortedPlayers' | 'algorithm' | 'matchHistoryStats'>;
} => {
...
const selectTeamsWithHistoryWithDebug = (
	...
	const maxPlayerScore = calculatePlayerScoreBreakdown(sortedPlayers[0]).totalScore;
	...
	const generateCombinations = (
		...
		if (remainingPlayers.length === 0) {
			...
			const teamASynergy = calculateTeamSynergyWithDetails(teamA, cache).synergy;
			const teamBSynergy = calculateTeamSynergyWithDetails(teamB, cache).synergy;

			const synergyDiff = Math.abs(teamASynergy - teamBSynergy);

			const normalizedSkillDiff = maxPlayerScore > 0 ? skillDiff / maxPlayerScore : 0;

			const maxPossibleSynergyDiff = 2;
			const normalizedSynergyDiff = synergyDiff / maxPossibleSynergyDiff;

			const combinedScore =
				skillWeight * normalizedSkillDiff + synergyWeight * normalizedSynergyDiff;
