<script lang="ts">
	...
	import { selectTeamsWithStats } from '$lib/utils/teamSelection';
	...
	let uploadedFiles = $derived(appStore.uploadedFiles);
	...
	async function handleFileUpload(files: File[]) {
		const toastId = toast.loading(`Processing ${files.length} file${files.length === 1 ? '' : 's'}...`);

		try {
			for (const file of files) {
				const { players: parsedPlayers, matches } = await parseXlsxData(file);
				appStore.addFileData(file.name, { players: parsedPlayers, matches });
			}

			toast.success(`${files.length} file${files.length === 1 ? '' : 's'} processed`);
		} catch (error) {
	...
	function handleRemoveFile(fileName: string) {
		appStore.removeFileData(fileName);
