...
const buildSynergyCache = (matchHistory: import('./types').Match[]): SynergyCache => {
...
const selectTeamsWithHistory = (
	...
	const generateCombinations = (
		...
		if (remainingPlayers.length === 0) {
			...
			const teamAScore = teamA.reduce((sum, p) => sum + calculatePlayerScore(p), 0);
			const teamBScore = teamB.reduce((sum, p) => sum + calculatePlayerScore(p), 0);
			...
			const combinedScore =
				SKILL_WEIGHT * normalizedSkillDiff + SYNERGY_WEIGHT * normalizedSynergyDiff;
